...
Players submit bids simultaneously. Highest bidder wins and pays their bid.
"""


class C(BaseConstants):
    NAME_IN_URL = 'sealed_bid_first_price'
...
class Group(BaseGroup):
    ...
    winning_bid = models.CurrencyField(initial=cu(0))
...
def set_payoffs(group: Group):
    ...
    highest_bid = max([player.bid for player in players])
    group.highest_bid = highest_bid

    if highest_bid < cu(C.RESERVE_PRICE):
        group.sold = False
        group.winning_bid = cu(0)
        for player in players:
            player.is_winner = False
            player.payoff = cu(0)
        return

    winners = [player for player in players if player.bid == highest_bid]
    winner = random.choice(winners)

    group.sold = True
    group.winning_bid = winner.bid

    for player in players:
        player.is_winner = player == winner
        if player.is_winner:
            player.payoff = max(cu(0), player.private_value - winner.bid)
        else:
            player.payoff = cu(0)
